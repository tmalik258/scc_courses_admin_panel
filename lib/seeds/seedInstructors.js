    ...
    for (const instructor of instructors) {
      ...
      if (authError) {
        ...
        if (authError.message.includes("already been registered")) {
          ...
          if (listError) throw new Error(`Supabase list users error: ${listError.message}`);

          const existingUser = users.users.find(user => user.email === instructor.email);
          if (!existingUser) throw new Error(`User with email ${instructor.email} not found`);
